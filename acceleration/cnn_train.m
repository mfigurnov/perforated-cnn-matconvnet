...
if isempty(opts.getBatchTrain)
  getBatchTrain = getBatch;
else
  getBatchTrain = opts.getBatchTrain;
end
...
for epoch=1:opts.numEpochs
  ...
  if ~isempty(opts.train) && prevLr ~= lr
    if opts.verbose
  ...
  for t=1:opts.batchSize:numel(train)
    ...
    if opts.miniBatchSize == 0
      ...
      for idx = 1:opts.miniBatchSize:numel(batch)
        idxEnd = min(numel(batch), idx + opts.miniBatchSize - 1);
        net.layers{end}.class = labels(idx:idxEnd) ;
        res = vl_simplenn(net, im(:,:,:,idx:idxEnd), one, res, ...
          'conserveMemory', opts.conserveMemory, ...
          'sync', opts.sync, ...
          'accumulateGradients', idx > 1) ;
        if idx == 1
          sz = size(res(end-1).x);
          resProb = zeros(sz(1),sz(2),sz(3),numel(batch),'like',res(end-1).x);
          loss = 0;
        end
        resProb(:,:,:,idx:idxEnd) = res(end-1).x;
        loss = loss + res(end).x;
      end
      res(end-1).x = resProb;  % aggregated labels
      res(end).x = loss;  % total loss
      net.layers{end}.class = labels ;
    end
    ...
    for l=1:numel(net.layers)
      ...
      net.layers{l}.filtersMomentum = opts.momentum * net.layers{l}.filtersMomentum;
      net.layers{l}.filtersMomentum = net.layers{l}.filtersMomentum ...
          - (lr * net.layers{l}.filtersLearningRate) * ...
          (opts.weightDecay * net.layers{l}.filtersWeightDecay) * net.layers{l}.filters;
      net.layers{l}.filtersMomentum = net.layers{l}.filtersMomentum ...
          - (lr * net.layers{l}.filtersLearningRate) / numel(batch) * res(l).dzdw{1} ;
      
      net.layers{l}.biasesMomentum = opts.momentum * net.layers{l}.biasesMomentum;
      net.layers{l}.biasesMomentum = net.layers{l}.biasesMomentum ...
  ...
  for t=1:opts.batchSize:numel(val)
    ...
    if opts.miniBatchSize == 0
      ...
      for idx = 1:opts.miniBatchSize:numel(batch)
        idxEnd = min(numel(batch), idx + opts.miniBatchSize - 1);
        net.layers{end}.class = labels(idx:idxEnd) ;
        res = vl_simplenn(net, im(:,:,:,idx:idxEnd), [], res, ...
          'disableDropout', true, ...
          'conserveMemory', opts.conserveMemory, ...
          'sync', opts.sync) ;
        if idx == 1
